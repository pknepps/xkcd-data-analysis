...
xkcd_sample <- xkcd |> sample_n(10) |> pull(transcript)
```
...
xkcd_date |> mutate(year = format(date, "%Y")) |> ggplot(aes(year)) + geom_bar(fill = "brown") + labs(title = "Number of Comics Posted by Year")
```
```{r comicsByWeekday}
...
specialComics |> mutate(year = format(date, "%Y")) |> ggplot(aes(year)) + geom_bar(fill = "red") + labs(title = "Special Comics by Year")
hasLink |> mutate(year = format(date, "%Y")) |> ggplot(aes(year)) + geom_bar(fill = "green") + labs(title = "Comics with Links by Year")
hasNews |> mutate(year = format(date, "%Y")) |> ggplot(aes(year)) + geom_bar(fill = "blue") + labs(title = "Comics with News by Year")
```
...
Turns out, Munroe stopped writing transcripts in 2016, not a single transcript was missed until 2015. Then the transcripts stopped overall. This was a very unfortunate surprise to our already unfortunate experience parsing the transcript. However, we still have data from the first 10 years of the comic.
...
```{r wordclouds}
parsed_comic <- xkcd_comma |> pull(transcript) |> parse_dialog()
words <- parsed_comic$scene |> append(parsed_comic$dialog)
head(words)
tail(words)
all_words <- lapply(words, \(x) str_split(x, r"{\s+|<[^>]*>+|\\n|[\\\/\~\@\#\$\%\^\&\*\(\)\-\_\=\+\[\]\}\{\,\.\?\/\'\"\;\:\|\>\<]}", simplify = TRUE))
all_words <- unlist(all_words)
all_words <- all_words[all_words != "" | all_words != "NA"]

all_words
all_words_tibble <- tibble(words=all_words)
all_words_tibble <- all_words_tibble |> group_by(words) |> summarise(word_count=n()) |> arrange(desc(word_count))
all_words_over_100 <- all_words_tibble |> filter(word_count > 100)

set.seed(42)
# The most common words
ggplot(all_words_over_100, aes(label = words, size = word_count)) +
  geom_text_wordcloud() +
  theme_minimal()
