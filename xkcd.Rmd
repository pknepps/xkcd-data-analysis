...
dialogue_regex <- regex(r"{(?m)^(\w+):([^\n]*)}", multiline = TRUE)
# dialogue_regex <- regex("^(\\w+):([^\n]*)", multiline = TRUE)
str_extract_all(test, r"{(?m)^(\w+):([^\n]*)}")
str_match_all(test, r"{(?m)^(\w+):([^\n]*)}")

my_data = 'CAT SAT ON A MOUSE
CAT
SAT
ON A MOUSE'
cat(my_data)
...
str_extract_all(my_data, r"{(?m)^.AT}")
...
```{r}
specialComics <- xkcd |> filter(extra_parts != " NA")
hasNews <- xkcd |> filter(news != " NA")
hasLink <- xkcd |> filter(link != " NA")
nrow(specialComics)
nrow(hasNews)
nrow(hasLink)
```

So there are 23 special comics. 54 comics with news announced, and 72 comics which have a link attached to the picture.
# Sources
