...
parse_dialog <- function(dialog) {
    ...
    step2.4 = str_remove_all(step2.3, bad_newlines)
    character_and_speech = str_match_all(step2.4, dialog_regex)
    return(character_and_speech)
}

# mutate_and_parse = function(df) {
#     transcript_split = df$transcript |> parse_dialog()
#     people = c()
#     say = c()
#     for (x in 1:length(transcript_split)) {
#         people[x] = cat(transcript_split[[x]][,2])
#         say[x] = cat(transcript_split[[x]][,3])
#     }
#     out = c(people, say)
#     return(people)
# }
...
mutate_and_parse = function(df) {
    return(df$transcript |> parse_dialog() |> map(as.data.frame) |> map(\(x) reduce(x$V2, paste, .init = "")))
}

data <- xkcd_date$transcript |> parse_dialog()
head(xkcd_date) |> mutate_and_parse()
head(xkcd_date)

head(data) |> map(\(x) reduce(x, \(y) paste(y, sep=" ")))
cat(data[[512]][,2])
head(data)
...
```{r}
xkcd_transcript_temp <- xkcd_date |> separate_longer_delim(transcript, regex(names_regex)) |> group_by(num, date, weekday, title, safe_title, alt, img, link, news, extra_parts) |> summarise(transcript = paste(transcript, collapse = " "))
xkcd_transcript_temp
```
# Analysis
## Dates
...
```{r}
#TODO: delete below single line
xkcd_transcript <- xkcd_date
xkcd_words <- xkcd_transcript |> separate_longer_delim(transcript, delim = regex(r"(\s+)"))
